# Spec: timeline-spec-matrix

## Purpose

Mode Matrice de la TimelinePage : grille spec × date représentant l'intensité d'activité de chaque spec dans le temps, avec panel droit de détail spec, drill-down vers le DetailPanel et navigation vers la SpecsPage.
...
- **THEN** la cellule affiche une couleur d'intensité basse (`bg-blue-100`)

#### Scenario: Cellule avec plusieurs changes
- **WHEN** plusieurs changes ont touché cette spec le même jour
- **THEN** la cellule affiche une intensité proportionnelle au nombre de changes (`bg-blue-300` pour 2, `bg-blue-500` pour 3+)
