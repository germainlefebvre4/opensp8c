...
L'utilisateur SHALL pouvoir cliquer sur une carte dans les colonnes **To Do**, **In Progress** ou **Done** pour ouvrir un panneau latéral (`DetailPanel`) affichant le détail complet du change. Un seul panneau peut être ouvert à la fois ; ouvrir un panneau ferme tout autre panneau précédemment ouvert (ExplorePanel inclus).
...
- **THEN** le `DetailPanel` s'ouvre à droite, affichant le détail du change correspondant
...
- **THEN** le panneau se ferme et le Kanban est affiché sans panneau latéral

### Requirement: Afficher la liste des tâches dans le DetailPanel
Le `DetailPanel` SHALL afficher la liste complète des tâches du change avec leur état (faite / non faite), lue depuis le fichier `tasks.md` via le backend.

#### Scenario: Change avec tasks.md
- **WHEN** le DetailPanel s'ouvre pour un change ayant un `tasks.md`
- **THEN** chaque tâche est affichée avec son texte et un indicateur visuel de complétion (`[x]` vs `[ ]`)
...
- **THEN** la réponse contient `name`, `kanban_status`, `tasks_done`, `tasks_total`, `tasks` (tableau d'objets `{ text, done }`), et `artifacts` (`{ proposal, design }` avec `null` si absent)
